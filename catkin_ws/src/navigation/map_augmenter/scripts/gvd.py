...
def brushfire(grid_map):
    ...
    while not Q.empty():
        ...
        for k1, k2 in offsets_8:
            if distances[i+k1, j+k2] == -1:
                Q.put([i+k1, j+k2])
                distances[i+k1, j+k2] = d + 2 #D. Manhattan
                #distances[i+k1, j+k2] = d + math.sqrt(2.0) #D. Euclideana
            else:
                distances[i+k1, j+k2] = min(distances[i+k1, j+k2], d+ 2) #D. Manhattan
                #distances[i+k1, j+k2] = min(distances[i+k1, j+k2], d+ math.sqrt(2.0)) #D. Euclideana
    return distances
